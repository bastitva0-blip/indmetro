 ...
 * Both lines: 6:00–22:00 daily
 * Peak: 10 min | Off-peak: 15 min
 */
...
export const getAllSchedules = (): TrainSchedule[] => {
  ...
  for (const line of ["blue", "red"] as const) {
    ...
    for (const dir of ["forward", "backward"] as const) {
      ...
      while (time <= toMin("22:00")) {
        ...
        const hw = getHeadwayMinutes(line, mock);
        schedules.push({
          id: `${line.toUpperCase()[0]}L-${dir === "forward" ? "F" : "B"}-${seq}`,
