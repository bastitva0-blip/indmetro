...
export const planRoute = (originId: string, destinationId: string, hasCard = false): PlannedRoute | null => {
  ...
  const l1t = getTravelTimeMinutes(oLine, originId, ix) ?? l1stops * 2;
  const l2t = getTravelTimeMinutes(dLine, ix, destinationId) ?? l2stops * 2;
  ...
  return {
    origin, destination,
    steps: [
      { type: "board", line: oLine, stationId: originId, stationName: origin.name, direction: `towards Kalupur (interchange)` },
      { type: "travel", line: oLine, numStops: l1stops, durationMinutes: l1t },
      { type: "interchange", stationId: ix, stationName: "Kalupur" },
      { type: "board", line: dLine, stationId: ix, stationName: "Kalupur", direction: l2dir },
