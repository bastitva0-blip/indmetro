...
export interface RouteStep {
  ...
  line?: "orange" | "aqua"; stationId?: string; stationName?: string;
...
const commonLine = (a: string, b: string): "orange" | "aqua" | null => {
  for (const l of ["orange", "aqua"] as const)
    if (LINE_STATIONS[l].includes(a) && LINE_STATIONS[l].includes(b)) return l;
  return null;
};

const getLineForStation = (id: string): "orange" | "aqua" | null =>
  LINE_STATIONS.orange.includes(id) ? "orange" : LINE_STATIONS.aqua.includes(id) ? "aqua" : null;
...
export const planRoute = (originId: string, destinationId: string, hasCard = false): PlannedRoute | null => {
  ...
  if (direct) {
    ...
    const travel = getTravelTimeMinutes(direct, originId, destinationId) ?? stops * 1.9;
  ...
  // Interchange via Sitabuldi
  const oLine = getLineForStation(originId), dLine = getLineForStation(destinationId);
  if (!oLine || !dLine) return null;
  const ix = "sitabuldi";
  if (!LINE_STATIONS[oLine].includes(ix) || !LINE_STATIONS[dLine].includes(ix)) return null;
  ...
  const d1 = getDistanceKm(originId, ix, oLine);
  const d2 = getDistanceKm(ix, destinationId, dLine);
  const l1t = getTravelTimeMinutes(oLine, originId, ix) ?? l1stops * 1.9;
  const l2t = getTravelTimeMinutes(dLine, ix, destinationId) ?? l2stops * 1.9;
  ...
  return {
    origin, destination,
    steps: [
      { type: "board", line: oLine, stationId: originId, stationName: origin.name, direction: "towards Sitabuldi (interchange)" },
      { type: "travel", line: oLine, numStops: l1stops, durationMinutes: l1t },
      { type: "interchange", stationId: ix, stationName: "Sitabuldi" },
      { type: "board", line: dLine, stationId: ix, stationName: "Sitabuldi", direction: l2dir },
      { type: "travel", line: dLine, numStops: l2stops, durationMinutes: l2t },
      { type: "alight", stationId: destinationId, stationName: destination.name },
    ],
    totalStations: l1stops + l2stops, totalTime: Math.round(wait + l1t + 3 + l2t),
    interchangeCount: 1, distanceKm: totalDist,
    fare: calculateFare(totalDist, false), discountedFare: calculateFare(totalDist, true),
    isDirect: false,
    departureTime: fmt(curr + wait), arrivalTime: fmt(curr + wait + l1t + 3 + l2t),
  };
};
