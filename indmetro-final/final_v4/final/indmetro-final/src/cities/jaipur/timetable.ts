/**
 * Jaipur Metro — Timetable
 *
 * Operating hours: 6:00 AM – 9:45 PM (daily)
 * Peak headway:   10 minutes
 * Off-peak:       15 minutes
 *
 * Source: JMRC official timetable (trainhelp.in / JMRC 2025)
 */
...
export const OPERATING_HOURS = { firstTrain: "06:00", lastTrain: "21:45" };
...
const generateSchedules = (): TrainSchedule[] => {
  ...
  for (const direction of ["forward", "backward"] as const) {
    ...
    while (t <= last) {
      ...
      schedules.push({
        id: `PINK-${direction === "forward" ? "F" : "B"}-${seq}`,
        line,
        direction,
        startTime: fmtMin(t),
        stations: stIds,
...
export const getNextTrainsAtStation = (
  ...
  for (const s of getAllSchedules().filter(
    ...
    if (away >= -1 && away <= 120) {
      results.push({
        schedule: s,
        arrivalTime: fmtMin(arrivalMin),
        minutesAway: Math.max(0, away),
      });
    }
