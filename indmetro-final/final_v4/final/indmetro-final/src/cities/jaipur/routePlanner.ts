 ...
 *
 * Single Pink Line, no interchanges. All routes are direct.
 */
...
export const planRoute = (
  ...
  const travelTime =
    getTravelTimeMinutes(line, originId, destinationId) ?? totalStations * 2.6;
  const fare = calculateFare(originId, destinationId, false);
  const discountedFare = calculateFare(originId, destinationId, true);
  ...
  // Direction label: towards terminal
