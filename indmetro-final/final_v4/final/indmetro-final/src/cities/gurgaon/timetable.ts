...
export interface TrainSchedule {
  id: string; line: "rapid"; direction: "forward" | "backward";
  startTime: string; stations: string[]; stationTimes: number[];
}
export const OPERATING_HOURS = { firstTrain: "06:05", lastTrain: "22:00" };
...
export const getAllSchedules = (): TrainSchedule[] => {
  ...
  for (const dir of ["forward", "backward"] as const) {
    ...
    while (time <= toMin(OPERATING_HOURS.lastTrain)) {
      ...
      schedules.push({ id: `RAPID-${dir === "forward" ? "F" : "B"}-${seq}`, line: "rapid", direction: dir, startTime: fmtMin(time), stations: stIds, stationTimes: cum });
