...
import { getTravelTimeMinutes } from "@/cities/pune/segmentTimings";
import { LINE_STATIONS, stations, getStationLine } from "@/cities/pune/metroData";

export type PuneLine = "purple" | "aqua";
...
export interface JourneyState {
  ...
  destinationStationId: string | null; line: PuneLine; stops: JourneyStop[];
...
const INITIAL: JourneyState = {
  ...
  destinationStationId: null, line: "purple", stops: [],
  currentStationId: null, nextStationId: null,
...
export function useJourneyTracker() {
  ...
  const startJourney = useCallback(async (
    boardingStationId: string, destinationStationId: string, line: PuneLine
  ) => {
    ...
    setState({
      active: true, boardingTime: new Date(), boardingStationId, destinationStationId,
      line, stops, currentStationId: boardingStationId,
          ...
          if (prevDest && elapsedMin >= prevDest.etaMinutes && !notifiedRef.current.has(destId)) {
            ...
            try {
              if (Notification.permission === "granted")
                new Notification("IndMetro — Pune", {
