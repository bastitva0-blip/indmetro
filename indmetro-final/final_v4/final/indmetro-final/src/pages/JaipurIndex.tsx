...
import { planRoute, PlannedRoute } from "@/cities/jaipur/routePlanner";
import { getCrowdEstimate } from "@/cities/jaipur/crowdSimulation";
import { getNextTrainsAtStation } from "@/cities/jaipur/timetable";
...
function JaipurMap({
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      ...
      // Pink Line — elevated portion (Mansarovar → Sindhi Camp)
      const elevatedIds = LINE_STATIONS.pink.slice(0, 8); // up to Sindhi Camp
      L.polyline(
        elevatedIds.map((id) => stations[id].coordinates as [number, number]),
        { color: LINE_COLORS.pink, weight: 5, opacity: 0.9 }
      ).addTo(map);

      // Pink Line — underground portion (Sindhi Camp → Badi Chaupar)
      const undergroundIds = LINE_STATIONS.pink.slice(7); // from Sindhi Camp
      L.polyline(
        undergroundIds.map((id) => stations[id].coordinates as [number, number]),
        { color: LINE_COLORS.pink, weight: 5, opacity: 0.6, dashArray: "6 4" }
      ).addTo(map);
      ...
      Object.values(stations).forEach((s) => {
        const marker = L.circleMarker(
          s.coordinates as [number, number],
          {
            ...
            color: "#fff",
            weight: 2,
            fillColor: s.isUnderground ? "#C2185B" : LINE_COLORS.pink,
            fillOpacity: 0.95,
          }
        ).addTo(map);

        // Underground icon hint
        const tooltipLabel = s.isUnderground
          ? `${s.name} 🚇`
          : s.name;
        marker.bindTooltip(tooltipLabel, { direction: "top", offset: [0, -8] });
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300">
          {LINE_NAMES.pink}
        </span>
      </div>
      ...
      {hasGoSmartCard && route.discountedFare && route.discountedFare < route.fare && (
        <p className="text-xs text-center text-pink-600 dark:text-pink-400">
          JMRC Smart Card discount applied
        </p>
      )}
      ...
      <button
        ...
        style={{ backgroundColor: LINE_COLORS.pink }}
      >
...
function StationSelect({
  ...
  // Stations in line order (West → East), not alphabetical
  const ops = LINE_STATIONS.pink.map((id) => stations[id]);
  return (
    <div className="flex flex-col gap-1">
      ...
      <select
        ...
        className="bg-muted border border-border rounded-xl px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-pink-500"
      >
...
export default function JaipurIndex() {
  ...
  const { journey, startJourney, endJourney, requestNotificationPermission } =
    useJourneyTracker();
  ...
  useEffect(() => {
    if (origin && dest && origin !== dest) {
      ...
      if (r) {
        setRoute(r);
        setRouteError(null);
        // Highlight all stations on the route
        const oIdx = LINE_STATIONS.pink.indexOf(origin);
        const dIdx = LINE_STATIONS.pink.indexOf(dest);
        if (oIdx !== -1 && dIdx !== -1) {
          const slice =
            oIdx < dIdx
              ? LINE_STATIONS.pink.slice(oIdx, dIdx + 1)
              : LINE_STATIONS.pink.slice(dIdx, oIdx + 1).reverse();
  ...
  const handleStart = useCallback(async () => {
    ...
    await startJourney(route.origin.id, route.destination.id, "pink");
  ...
  const nextTrains = selectedStation
    ? [
        ...getNextTrainsAtStation(selectedStation, "pink", "forward", 2),
        ...getNextTrainsAtStation(selectedStation, "pink", "backward", 2),
      ]
    : [];
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-background">
      ...
      <OfflineIndicator />
      <JaipurMap highlightIds={highlightIds} onStationClick={handleMapClick} />
      ...
      <div className="absolute top-0 left-0 right-0 z-[1200] px-3 pt-3 flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-xl px-3 h-9 shadow">
          <Train className="w-4 h-4 shrink-0" style={{ color: LINE_COLORS.pink }} />
          <span className="text-sm font-semibold">Jaipur Metro</span>
        ...
        <div className="overflow-y-auto max-h-[55vh] px-4 pt-4 pb-8 space-y-4">
          ...
          {activePanel === "stations" && (
            <div className="flex flex-col gap-2">
              {/* Stations in Pink Line order */}
              {LINE_STATIONS.pink.map((id) => {
                const s = stations[id];
                return (
                  <button
                    key={s.id}
                    ...
                    className={cn(
                      ...
                      selectedStation === s.id
                        ? "bg-pink-50 dark:bg-pink-900/20 border border-pink-300 dark:border-pink-700"
                  ...
                  >
                    <div>
                      <p className="text-sm font-medium flex items-center gap-1.5">
                        {s.name}
                        {s.isUnderground && (
                          <span className="text-[10px] bg-pink-100 dark:bg-pink-900/40 text-pink-700 dark:text-pink-300 px-1.5 py-0.5 rounded-full">
                            underground
                          </span>
                        )}
                      </p>
