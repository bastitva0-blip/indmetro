...
import { stations, getStationOptions, LINE_STATIONS, LINE_COLORS, LINE_NAMES, OPERATIONAL_STATIONS } from "@/cities/agra/metroData";
import { planRoute, PlannedRoute } from "@/cities/agra/routePlanner";
import { getCrowdEstimate } from "@/cities/agra/crowdSimulation";
import { getNextTrainsAtStation } from "@/cities/agra/timetable";
import { useGoSmartCard } from "@/contexts/GoSmartCardContext";
import ThemeToggle from "@/components/ThemeToggle";
import OfflineIndicator from "@/components/OfflineIndicator";
import { cn } from "@/lib/utils";

function AgraMap({ highlightIds, onStationClick }: { highlightIds: string[]; onStationClick: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      const L = mod.default;
      const map = L.map(containerRef.current!, { center: [27.188, 78.010], zoom: 13, zoomControl: false });
      ...
      // Yellow operational
      const yOp = ["taj_east_gate","basai","fatehabad_road","taj_mahal","dr_ambedkar_chowk","mankameshwar"];
      L.polyline(yOp.map(id => stations[id].coordinates as [number,number]), { color: LINE_COLORS.yellow, weight: 5, opacity: 0.9 }).addTo(map);
      // Yellow WIP
      const yWip = ["mankameshwar","medical_college","agra_college","raja_ki_mandi","rbs_college","isbt_agra","guru_ka_taal","sikandra"];
      L.polyline(yWip.map(id => stations[id].coordinates as [number,number]), { color: LINE_COLORS.yellow, weight: 4, opacity: 0.4, dashArray: "8 6" }).addTo(map);
      // Blue WIP
      const bAll = LINE_STATIONS.blue.map(id => stations[id].coordinates as [number,number]);
      L.polyline(bAll, { color: LINE_COLORS.blue, weight: 4, opacity: 0.4, dashArray: "8 6" }).addTo(map);

      Object.values(stations).forEach((s) => {
        const isOp = OPERATIONAL_STATIONS.has(s.id);
        const primary = s.lines[0] as "yellow" | "blue";
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-1">
          {route.steps.filter(s => s.type === "board").map((s, i) => (
            <span key={i} className={cn("text-xs px-2 py-0.5 rounded-full font-medium", LINE_BADGE[s.line!])}>{LINE_NAMES[s.line as "yellow" | "blue"]}</span>
          ))}
        </div>
      </div>
      {route.interchangeCount > 0 && <p className="text-xs text-muted-foreground flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Interchange at Agra College</p>}
...
export default function AgraIndex() {
  ...
  useEffect(() => {
    if (origin && dest && origin !== dest) {
      ...
      if (r) {
        ...
        const yArr = LINE_STATIONS.yellow;
        const oi = yArr.indexOf(origin), di = yArr.indexOf(dest);
        if (oi !== -1 && di !== -1) setHighlightIds(oi < di ? yArr.slice(oi, di+1) : yArr.slice(di, oi+1).reverse());
  ...
  const handleStart = useCallback(async () => {
    ...
    const line = (route.steps.find(s => s.line)?.line ?? "yellow") as "yellow" | "blue";
  ...
  const nextTrains = selectedStation && OPERATIONAL_STATIONS.has(selectedStation) && LINE_STATIONS.yellow.includes(selectedStation)
    ? [...getNextTrainsAtStation(selectedStation, "yellow", "forward", 2), ...getNextTrainsAtStation(selectedStation, "yellow", "backward", 2)] : [];
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-background">
      ...
      <OfflineIndicator />
      <AgraMap highlightIds={highlightIds} onStationClick={handleMapClick} />
      ...
      <div className="absolute top-0 left-0 right-0 z-[1200] px-3 pt-3 flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-xl px-3 h-9 shadow">
          ...
          <span className="text-sm font-semibold">Agra Metro</span>
          <div className="ml-auto flex gap-1">
            <span className="text-[10px] bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 px-1.5 py-0.5 rounded-full">Yellow Live</span>
            <span className="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded-full">Blue UC</span>
          </div>
        </div>
        <ThemeToggle />
      </div>
                ...
                <div className="mt-2 bg-muted/40 rounded-xl p-3">
                  ...
                  {nextTrains.map((t, i) => (
                    <div key={i} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                      <span className="text-muted-foreground">{t.schedule.direction==="forward"?"→ Taj East Gate":"← Sikandra"}</span>
