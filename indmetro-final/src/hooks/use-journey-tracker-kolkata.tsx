...
import { getTravelTimeMinutes } from "@/cities/kolkata/segmentTimings";
import { LINE_STATIONS, stations } from "@/cities/kolkata/metroData";
...
const INITIAL: JourneyState = {
  ...
  line: "blue" as "blue" | "green" | "orange" | "purple",
  stops: [],
...
export function useJourneyTracker() {
  ...
  const startJourney = useCallback(async (
    ...
    const stops: JourneyStop[] = ordered.map((id: string) => ({
      ...
      etaMinutes: getTravelTime(line, boardingStationId, id) ?? 0,
    }));
          ...
          if (prevDest && elapsedMin >= prevDest.etaMinutes && !notifiedRef.current.has(destId)) {
            ...
            try {
              if (Notification.permission === "granted")
                new Notification("IndMetro — Patna", {
