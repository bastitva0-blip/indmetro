...
import { getTravelTimeMinutes } from "@/cities/jaipur/segmentTimings";
import { LINE_STATIONS, stations } from "@/cities/jaipur/metroData";

export type JaipurLine = "pink";
  ...
  const startJourney = useCallback(
    async (
      ...
      const stops: JourneyStop[] = ordered.map((id) => ({
        ...
        etaMinutes:
          getTravelTimeMinutes(line, boardingStationId, id) ?? 0,
      }));
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setState((prev) => {
        ...
        const elapsedMin =
          (Date.now() - prev.boardingTime.getTime()) / 60000;
        const remaining = prev.stops.filter(
          (s) => s.etaMinutes > elapsedMin
        );
        ...
        // Notify 1 station before destination
        ...
        if (destId) {
          ...
          if (
            prevDest &&
            elapsedMin >= prevDest.etaMinutes &&
            ...
            try {
              if (Notification.permission === "granted")
                new Notification("IndMetro — Jaipur", {
        ...
        const currentStationId =
          passed[passed.length - 1]?.stationId ?? prev.boardingStationId;
        return {
          ...prev,
          stops: remaining,
          currentStationId,
