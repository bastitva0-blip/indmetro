...
export interface TrainSchedule {
  id: string; line: "blue" | "red"; direction: "forward" | "backward";
  startTime: string; stations: string[]; stationTimes: number[];
}

// Operational hours: 8:00 AM – 10:00 PM (per PMRC / Wikipedia)
export const OPERATING_HOURS = { firstTrain: "08:00", lastTrain: "22:00" };
...
export const getAllSchedules = (): TrainSchedule[] => {
  ...
  for (const line of ["blue"] as const) {
    // Only Blue Line operational; Red Line all WIP
    ...
    for (const dir of ["forward", "backward"] as const) {
      ...
      while (time <= toMin(OPERATING_HOURS.lastTrain)) {
        const hw = PEAK_HEADWAY[line]; // fixed 20 min
        schedules.push({
