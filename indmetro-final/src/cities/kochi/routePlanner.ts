...
export interface PlannedRoute {
  ...
  distanceKm: number; fare: number; discountedFare: number; isDirect: true;
...
/**
 * Single Blue Line — all routes are direct (no interchange ever needed).
 */
export const planRoute = (originId: string, destinationId: string, hasCard = false): PlannedRoute | null => {
  ...
  const arr = LINE_STATIONS.blue;
  const fi = arr.indexOf(originId), ti = arr.indexOf(destinationId);
  if (fi === -1 || ti === -1) return null;

  const stops = Math.abs(ti - fi);
  const distKm = getDistanceKm(originId, destinationId);
  const travel = getTravelTimeMinutes(originId, destinationId) ?? stops * 2.5;
  const now = getISTDate();
  const wait = getHeadwayMinutes(now) / 2;
  ...
  return {
    ...
    fare: calculateFare(distKm, false),
    discountedFare: calculateFare(distKm, true),
    isDirect: true,
    departureTime: fmt(curr + wait),
    arrivalTime: fmt(curr + wait + travel),
  };
};
