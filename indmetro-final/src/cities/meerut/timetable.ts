...
export interface TrainSchedule { id: string; line:"metro"; direction:"forward"|"backward"; startTime:string; stations:string[]; stationTimes:number[]; }
...
export const getAllSchedules = (): TrainSchedule[] => {
  ...
  for (const dir of ["forward","backward"] as const) {
    ...
    while (time <= toMin(OPERATING_HOURS.lastTrain)) {
      ...
      schedules.push({ id:`METRO-${dir==="forward"?"F":"B"}-${seq}`, line:"metro", direction:dir, startTime:fmtMin(time), stations:stIds, stationTimes:cum });
