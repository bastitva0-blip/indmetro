...
export const OPERATING_HOURS = { firstTrain: "06:00", lastTrain: "23:00" };
...
const isPeak = (minSinceMidnight: number): boolean => {
  const hour = Math.floor(minSinceMidnight / 60);
  const now  = getISTDate();
  const day  = now.getDay();
  return day >= 1 && day <= 5 && ((hour >= 8 && hour < 11) || (hour >= 17 && hour < 21));
};

const generateSchedules = (): TrainSchedule[] => {
  const schedules: TrainSchedule[] = [];
  for (const line of ["purple", "aqua"] as const) {
    ...
    for (const direction of ["forward", "backward"] as const) {
      ...
      while (t <= last) {
        const headway = isPeak(t) ? PEAK_HEADWAY_MINUTES[line] : OFF_PEAK_HEADWAY_MINUTES[line];
        schedules.push({
