...
import { stations, getStationOptions, LINE_STATIONS, LINE_COLORS, LINE_NAMES, OPERATIONAL_STATIONS } from "@/cities/hyderabad/metroData";
import { planRoute, PlannedRoute } from "@/cities/hyderabad/routePlanner";
import { getCrowdEstimate } from "@/cities/hyderabad/crowdSimulation";
import { getNextTrainsAtStation } from "@/cities/hyderabad/timetable";
import { useGoSmartCard } from "@/contexts/GoSmartCardContext";
import ThemeToggle from "@/components/ThemeToggle";
import OfflineIndicator from "@/components/OfflineIndicator";
import { cn } from "@/lib/utils";

function HyderabadMap({ highlightIds, onStationClick }: { highlightIds: string[]; onStationClick: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      const L = mod.default;
      const map = L.map(containerRef.current!, { center: [17.387, 78.490], zoom: 13, zoomControl: false });
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>', maxZoom: 19,
      }).addTo(map);
      L.control.zoom({ position: "bottomright" }).addTo(map);

      // Red Line — all operational
      const redCoords = LINE_STATIONS.red.map(id => stations[id].coordinates as [number,number]);
      L.polyline(redCoords, { color: LINE_COLORS.red, weight: 5, opacity: 0.9 }).addTo(map);
      // Blue Line — all operational
      ...
      // Green Line — 9 operational
      const greenOp = LINE_STATIONS.green.slice(0, 9).map(id => stations[id].coordinates as [number,number]);
      L.polyline(greenOp, { color: LINE_COLORS.green, weight: 5, opacity: 0.9 }).addTo(map);
      // Green Line — 6 WIP
      const greenWip = LINE_STATIONS.green.slice(8).map(id => stations[id]?.coordinates as [number,number]).filter(Boolean);
      L.polyline(greenWip, { color: LINE_COLORS.green, weight: 4, opacity: 0.4, dashArray: "8 6" }).addTo(map);

      Object.values(stations).forEach((s) => {
        const primary = s.lines[0] as "red" | "blue" | "green";
  ...
  useEffect(() => {
    import("leaflet").then(() => {
      Object.entries(markersRef.current).forEach(([id, marker]) => {
        ...
        else marker.setStyle({ fillColor: s.isWIP ? "#9CA3AF" : LINE_COLORS[primary], radius: s.isInterchange ? 9 : s.isWIP ? 5 : 7, weight: s.isInterchange ? 3 : 2 });
      });
    });
  }, [highlightIds]);
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-1">
          {route.steps.filter(s => s.type === "board").map((s, i) => (
            <span key={i} className={cn("text-xs px-2 py-0.5 rounded-full font-medium", LINE_BADGE[s.line!])}>{LINE_NAMES[s.line as "red" | "blue" | "green"]}</span>
          ))}
        </div>
      </div>
      {route.interchangeCount > 0 && <p className="text-xs text-muted-foreground flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Interchange at {route.steps.find(s => s.type === "interchange")?.stationName}</p>}
...
export default function HyderabadIndex() {
  ...
  useEffect(() => {
    if (origin && dest && origin !== dest) {
      ...
      if (r) {
        ...
        else setHighlightIds([origin, dest]);      } else { setRoute(null); setRouteError("No route between these stations."); }
  ...
  const handleStart = useCallback(async () => {
    ...
    const line = (route.steps.find(s => s.line)?.line ?? "blue") as "red" | "blue" | "green";
  ...
  const selectedLine = selectedStation
    ? (LINE_STATIONS.red.includes(selectedStation) ? "red" : LINE_STATIONS.blue.includes(selectedStation) ? "blue" : "green") as "red" | "blue" | "green"
    : "red";
  const nextTrains = selectedStation && OPERATIONAL_STATIONS.has(selectedStation)
    ? [...getNextTrainsAtStation(selectedStation, selectedLine, "forward", 2),
       ...getNextTrainsAtStation(selectedStation, selectedLine, "backward", 2)] : [];
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-background">
      ...
      <OfflineIndicator />
      <HyderabadMap highlightIds={highlightIds} onStationClick={handleMapClick} />
      ...
      <div className="absolute top-0 left-0 right-0 z-[1200] px-3 pt-3 flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-xl px-3 h-9 shadow">
          <Train className="w-4 h-4 text-red-500 shrink-0" />
          <span className="text-sm font-semibold">Hyderabad Metro</span>
          <div className="ml-auto flex gap-1">
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS.red }}>Red 26 live</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS.blue }}>Blue 23 live</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS.green }}>Green 9 live</span>
            <span className="text-[10px] bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 px-1.5 py-0.5 rounded-full">6 UC</span>
          </div>
        </div>
        <ThemeToggle />
      </div>
                ...
                return (
                  <button key={s.id} onClick={() => setSelectedStation(s.id)} className={cn("flex items-center justify-between px-3 py-2.5 rounded-xl text-left transition-colors", selectedStation===s.id?"bg-primary/10 border border-primary/30":"bg-muted/40 hover:bg-muted")}>
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{s.isWIP ? "🚧 Under construction" : `${LINE_NAMES[primary]}${s.isInterchange?" · Interchange":""}`}</p>
                    </div>
                    {isOp && crowd && selectedStation === s.id && (
                      <span className={cn("text-xs px-2 py-0.5 rounded-full",
                        crowd.level==="low"&&"bg-green-100 dark:bg-green-900/30 text-green-700",
                        crowd.level==="moderate"&&"bg-red-100 dark:bg-red-900/30 text-red-700",
                  ...
                  {nextTrains.map((t, i) => (
                    <div key={i} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                      <span className="text-muted-foreground">
                        {t.schedule.line === "red"
                          ? (t.schedule.direction === "forward" ? "→ LB Nagar" : "← Miyapur")
                          : t.schedule.line === "blue"
                            ? (t.schedule.direction === "forward" ? "→ Raidurg" : "← Nagole")
                            : (t.schedule.direction === "forward" ? "→ Falaknuma" : "← JBS")}
                      </span>
