...
import { planRoute, type PlannedRoute } from "@/cities/mumbai/routePlanner";
import { getNextTrainsAtStation } from "@/cities/mumbai/timetable";
import { getCrowdEstimate } from "@/cities/mumbai/crowdSimulation";
import { getStationOptions } from "@/cities/mumbai/metroData";

type MumbaiLine = "line1" | "line2a" | "line3" | "line7" | "line9";

const LINE_BADGE: Record<string, string> = {
  line1:  "bg-blue-100  dark:bg-blue-900/30  text-blue-700  dark:text-blue-300",
  line2a: "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300",
  line3:  "bg-cyan-100  dark:bg-cyan-900/30  text-cyan-700  dark:text-cyan-300",
  line7:  "bg-red-100   dark:bg-red-900/30   text-red-700   dark:text-red-300",
  line9:  "bg-pink-100  dark:bg-pink-900/30  text-pink-700  dark:text-pink-300",
};

// ── Map Component ─────────────────────────────────────────────────────────────
function MumbaiMap({ onStationClick, highlightIds }: { onStationClick: (id: string) => void; highlightIds: string[] }) {
  ...
  useEffect(() => {
    ...
    if (!L) return;
    const map = L.map(mapRef.current, { center: [19.076, 72.877], zoom: 12, zoomControl: true, attributionControl: false });
    ...
    for (const line of ["line1","line2a","line3","line7","line9"] as MumbaiLine[]) {
      ...
      const isDash = line === "line3"; // underground shown with subtle dash
      L.polyline(coords, { color: LINE_COLORS[line], weight: line === "line3" ? 4 : 5, opacity: 0.9, dashArray: isDash ? "6 3" : undefined }).addTo(map);
    }

    // Station markers
    Object.values(stations).forEach(s => {
      const primary = s.lines[0];
      const color = LINE_COLORS[primary as MumbaiLine] ?? "#888";
...
function RouteCard({ route }: { route: PlannedRoute }) {
  return (
    <div className="bg-muted/50 border border-border rounded-xl p-3 flex flex-col gap-2">
      ...
      {route.note && (
        <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{route.note}</p>
      )}
      {route.steps.filter(s => s.type !== "travel").map((s, i) => (
        <div key={i} className="flex items-center gap-2 text-xs">
          {s.type === "board" && <><div className="w-2 h-2 rounded-full shrink-0" style={{ background: LINE_COLORS[s.line as MumbaiLine] }} /><span className="font-medium">{s.stationName}</span><span className="text-muted-foreground">{s.direction}</span></>}
...
export default function MumbaiIndex() {
  ...
  const [activeStationLine, setActiveStationLine] = useState<MumbaiLine>("line1");

  const handleMapClick = useCallback((id: string) => {
    setSelectedStation(id);
    const s = stations[id];
    if (s) setActiveStationLine(s.lines[0] as MumbaiLine);
  ...
  const stLine = selectedSt?.lines[0] as MumbaiLine | undefined;
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border bg-card shrink-0">
        ...
        <span className="text-sm font-semibold">Mumbai Metro</span>
        <div className="ml-auto flex gap-1 flex-wrap">
          {(["line1","line2a","line3","line7","line9"] as MumbaiLine[]).map(l => (
            <span key={l} className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS[l] }}>
              {l === "line9" ? "L9 NEW" : LINE_NAMES[l].split(" ")[0]}
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="h-[42vh] shrink-0 px-3 pt-2">
        <MumbaiMap onStationClick={handleMapClick} highlightIds={highlightIds} />
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
        {activePanel === "route" && (
          <>
            ...
            <p className="text-xs text-center text-muted-foreground">10% off with Mumbai Metro NCMC Card</p>
          </>
        )}
        ...
        {activePanel === "stations" && (
          <>
            ...
            {LINE_STATIONS[activeStationLine].map(id => {
              ...
              if (!s) return null;
              return (
                <button key={id} onClick={() => { setSelectedStation(id); setActiveStationLine(s.lines[0] as MumbaiLine); }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-1 flex-wrap mt-0.5">
                      ...
                      {s.isAtGrade && <span className="text-[10px] bg-slate-100 dark:bg-slate-900/30 text-slate-600 dark:text-slate-400 px-1.5 rounded-full">At-grade</span>}
                    </div>
                  </div>
        ...
        {activePanel === "info" && (
          <div className="flex flex-col gap-3">
                ...
                <div key={l} className="flex items-center gap-2 py-1.5 border-b border-border last:border-0">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {LINE_STATIONS[l].length} stations · {l === "line9" ? "Opened Apr 2026 🆕" : "Operational"}
                    </p>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-muted/50 border border-border rounded-xl p-3 flex flex-col gap-1.5">
              ...
              {[
                { name: "Marol Naka", lines: ["line1","line3"], note: "Blue ↔ Aqua" },
                { name: "Dahisar East", lines: ["line2a","line7","line9"], note: "Yellow ↔ Red ↔ Line 9" },
                { name: "Gundavali / Andheri", lines: ["line7","line1"], note: "Red → Blue (skywalk ~7 min)" },
              ].map((ix, i) => (
                <div key={i} className="flex items-center gap-2 py-1">
                  <div className="flex gap-0.5">
                    {ix.lines.map(l => <div key={l} className="w-2 h-2 rounded-full" style={{ background: LINE_COLORS[l as MumbaiLine] }} />)}
                  </div>
                  <div>
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3">
              ...
              <p className="text-[10px] text-amber-700 dark:text-amber-400 leading-relaxed">Line 1 uses Mumbai Metro One Card (MMOPL). Lines 2A/7/9 and Line 3 use NCMC. Cards are not interchangeable across all lines yet — buy NCMC for widest coverage.</p>
            </div>
          </div>
        )}
