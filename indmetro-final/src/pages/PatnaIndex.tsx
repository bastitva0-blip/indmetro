...
import { stations, getStationOptions, LINE_STATIONS, LINE_COLORS, LINE_NAMES, OPERATIONAL_STATIONS } from "@/cities/patna/metroData";
import { planRoute, PlannedRoute } from "@/cities/patna/routePlanner";
import { getCrowdEstimate } from "@/cities/patna/crowdSimulation";
import { getNextTrainsAtStation } from "@/cities/patna/timetable";
import { useGoSmartCard } from "@/contexts/GoSmartCardContext";
import ThemeToggle from "@/components/ThemeToggle";
import OfflineIndicator from "@/components/OfflineIndicator";
import { cn } from "@/lib/utils";

function PatnaMap({ highlightIds, onStationClick }: { highlightIds: string[]; onStationClick: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      const L = mod.default;
      const map = L.map(containerRef.current!, { center: [25.592, 85.162], zoom: 13, zoomControl: false });
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>', maxZoom: 19,
      }).addTo(map);
      L.control.zoom({ position: "bottomright" }).addTo(map);

      // Blue Line — operational section (Bhootnath → Zero Mile → New ISBT)
      const blueOp = ["bhootnath", "zero_mile", "patliputra_bus_terminal"];
      L.polyline(blueOp.map(id => stations[id].coordinates as [number,number]), { color: LINE_COLORS.blue, weight: 5, opacity: 0.9 }).addTo(map);
      // Blue Line — WIP (Patna Junction → … → Khemnichak → Bhootnath)
      const blueWip = LINE_STATIONS.blue.filter(id => !["bhootnath","zero_mile","patliputra_bus_terminal"].includes(id));
      blueWip.push("bhootnath"); // connect WIP to operational
      ...
      // Red Line — all WIP
      const redAll = LINE_STATIONS.red.map(id => stations[id].coordinates as [number,number]);
      L.polyline(redAll, { color: LINE_COLORS.red, weight: 4, opacity: 0.4, dashArray: "8 6" }).addTo(map);
...
export default function PatnaIndex() {
  ...
  const nextTrains = selectedStation && OPERATIONAL_STATIONS.has(selectedStation) && LINE_STATIONS.blue.includes(selectedStation)
    ? [...getNextTrainsAtStation(selectedStation, "blue", "forward", 2), ...getNextTrainsAtStation(selectedStation, "blue", "backward", 2)] : [];
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-background">
      ...
      <OfflineIndicator />
      <PatnaMap highlightIds={highlightIds} onStationClick={handleMapClick} />
      ...
      <div className="absolute top-0 left-0 right-0 z-[1200] px-3 pt-3 flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-xl px-3 h-9 shadow">
          ...
          <span className="text-sm font-semibold">Patna Metro</span>
          <div className="ml-auto flex gap-1">
            <span className="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded-full">Blue Live</span>
            <span className="text-[10px] bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded-full">Red UC</span>
          </div>
        </div>
        <ThemeToggle />
      </div>
                ...
                <div className="mt-2 bg-muted/40 rounded-xl p-3">
                  ...
                  {nextTrains.map((t, i) => (
                    <div key={i} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                      <span className="text-muted-foreground">{t.schedule.direction==="forward"?"→ Patna Junction":"← New ISBT"}</span>
