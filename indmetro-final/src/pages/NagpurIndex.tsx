...
import { stations, getStationOptions, LINE_STATIONS, LINE_COLORS, LINE_NAMES, OPERATIONAL_STATIONS } from "@/cities/nagpur/metroData";
import { planRoute, PlannedRoute } from "@/cities/nagpur/routePlanner";
import { getCrowdEstimate } from "@/cities/nagpur/crowdSimulation";
import { getNextTrainsAtStation } from "@/cities/nagpur/timetable";
import { useGoSmartCard } from "@/contexts/GoSmartCardContext";
import ThemeToggle from "@/components/ThemeToggle";
import OfflineIndicator from "@/components/OfflineIndicator";
import { cn } from "@/lib/utils";

function NagpurMap({ highlightIds, onStationClick }: { highlightIds: string[]; onStationClick: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      const L = mod.default;
      const map = L.map(containerRef.current!, { center: [21.145, 79.080], zoom: 13, zoomControl: false });
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>', maxZoom: 19,
      }).addTo(map);
      L.control.zoom({ position: "bottomright" }).addTo(map);

      // Orange Line — all 18 stations operational
      const orangeCoords = LINE_STATIONS.orange.map(id => stations[id].coordinates as [number,number]);
      L.polyline(orangeCoords, { color: LINE_COLORS.orange, weight: 5, opacity: 0.9 }).addTo(map);
      // Aqua Line — all 20 stations (+ shared Sitabuldi) operational
      ...
      Object.values(stations).forEach((s) => {
        const primary = s.lines[0] as "orange" | "aqua";
...
const LINE_BADGE: Record<string, string> = {
  blue: "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300",
  red: "bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300",
};
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-1">
          {route.steps.filter(s => s.type === "board").map((s, i) => (
            <span key={i} className={cn("text-xs px-2 py-0.5 rounded-full font-medium", LINE_BADGE[s.line!])}>{LINE_NAMES[s.line as "orange" | "aqua"]}</span>
          ))}
        </div>
      </div>
...
export default function NagpurIndex() {
  ...
  useEffect(() => {
    if (origin && dest && origin !== dest) {
      ...
      if (r) {
        ...
        const bArr = LINE_STATIONS.orange;
  ...
  const handleStart = useCallback(async () => {
    ...
    const line = (route.steps.find(s => s.line)?.line ?? "orange") as "orange" | "aqua";
  ...
  const selectedLine = selectedStation
    ? (LINE_STATIONS.orange.includes(selectedStation) ? "orange" : "aqua") as "orange" | "aqua"
    : "orange";
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-background">
      ...
      <OfflineIndicator />
      <NagpurMap highlightIds={highlightIds} onStationClick={handleMapClick} />
      ...
      <div className="absolute top-0 left-0 right-0 z-[1200] px-3 pt-3 flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-xl px-3 h-9 shadow">
          ...
          <span className="text-sm font-semibold">Nagpur Metro</span>
          <div className="ml-auto flex gap-1">
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS.orange }}>18 live</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS.aqua }}>20 live</span>
          </div>
        </div>
        <ThemeToggle />
      </div>
                  ...
                  <button key={s.id} onClick={() => setSelectedStation(s.id)} className={cn("flex items-center justify-between px-3 py-2.5 rounded-xl text-left transition-colors", selectedStation===s.id?"bg-primary/10 border border-primary/30":"bg-muted/40 hover:bg-muted")}>
                    ...
                    {isOp && crowd && selectedStation === s.id && (
                      <span className={cn("text-xs px-2 py-0.5 rounded-full",
                        ...
                        crowd.level==="moderate"&&"bg-orange-100 dark:bg-orange-900/30 text-orange-700",
                        crowd.level==="high"&&"bg-orange-100 dark:bg-orange-900/30 text-orange-700",
                        crowd.level==="very-high"&&"bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700",
                      )}>{crowd.level}</span>
                    )}
                  </button>
                );
                  ...
                  {nextTrains.map((t, i) => (
                    <div key={i} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                      <span className="text-muted-foreground">
                        {t.schedule.line === "orange"
                          ? (t.schedule.direction === "forward" ? "→ Khapri" : "← Automotive Square")
                          : (t.schedule.direction === "forward" ? "→ Hingna Mount View" : "← Prajapati Nagar")}
                      </span>
