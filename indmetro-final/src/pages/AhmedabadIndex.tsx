...
import { stations, getStationOptions, LINE_STATIONS, LINE_COLORS, LINE_NAMES, OPERATIONAL_STATIONS } from "@/cities/ahmedabad/metroData";
import { planRoute, PlannedRoute } from "@/cities/ahmedabad/routePlanner";
import { getCrowdEstimate } from "@/cities/ahmedabad/crowdSimulation";
import { getNextTrainsAtStation } from "@/cities/ahmedabad/timetable";
import { useGoSmartCard } from "@/contexts/GoSmartCardContext";
import ThemeToggle from "@/components/ThemeToggle";
import OfflineIndicator from "@/components/OfflineIndicator";
import { cn } from "@/lib/utils";

function AhmedabadMap({ highlightIds, onStationClick }: { highlightIds: string[]; onStationClick: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      const L = mod.default;
      const map = L.map(containerRef.current!, { center: [23.035, 72.580], zoom: 13, zoomControl: false });
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>', maxZoom: 19,
      }).addTo(map);
      L.control.zoom({ position: "bottomright" }).addTo(map);

      // Blue Line — operational (Vastral Gam → Gurukul Road)
      const blueOp = LINE_STATIONS.blue.filter(id => !stations[id]?.isWIP);
      L.polyline(blueOp.map(id => stations[id].coordinates as [number,number]), { color: LINE_COLORS.blue, weight: 5, opacity: 0.9 }).addTo(map);
      // Blue Line — WIP (Thaltej section)
      const blueWip = ["gurukul_road", "thaltej_crossing", "sola_rd", "thaltej_gam"];
      L.polyline(blueWip.map(id => stations[id].coordinates as [number,number]), { color: LINE_COLORS.blue, weight: 4, opacity: 0.4, dashArray: "8 6" }).addTo(map);
      // Red Line — operational (APMC → Motera)
      const redOp = LINE_STATIONS.red.filter(id => !stations[id]?.isWIP);
      L.polyline(redOp.map(id => stations[id].coordinates as [number,number]), { color: LINE_COLORS.red, weight: 5, opacity: 0.9 }).addTo(map);
      // Red Line — WIP (Phase 2)
      const redWip = ["motera", "chandkheda", "ranip"];
      L.polyline(redWip.map(id => stations[id].coordinates as [number,number]), { color: LINE_COLORS.red, weight: 4, opacity: 0.4, dashArray: "8 6" }).addTo(map);

      Object.values(stations).forEach((s) => {
        const primary = s.lines[0] as "blue" | "red";
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-1">
          {route.steps.filter(s => s.type === "board").map((s, i) => (
            <span key={i} className={cn("text-xs px-2 py-0.5 rounded-full font-medium", LINE_BADGE[s.line!])}>{LINE_NAMES[s.line as "blue" | "red"]}</span>
          ))}
        </div>
      </div>
...
export default function AhmedabadIndex() {
  ...
  const handleStart = useCallback(async () => {
    ...
    const line = (route.steps.find(s => s.line)?.line ?? "blue") as "blue" | "red";
  ...
  const selectedLine = selectedStation
    ? (LINE_STATIONS.blue.includes(selectedStation) ? "blue" : "red") as "blue" | "red"
    : "blue";
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-background">
      ...
      <OfflineIndicator />
      <AhmedabadMap highlightIds={highlightIds} onStationClick={handleMapClick} />
      ...
      <div className="absolute top-0 left-0 right-0 z-[1200] px-3 pt-3 flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-xl px-3 h-9 shadow">
          ...
          <span className="text-sm font-semibold">Ahmedabad Metro</span>
          <div className="ml-auto flex gap-1">
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS.blue }}>Blue 18 live</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: LINE_COLORS.red }}>Red 16 live</span>
            <span className="text-[10px] bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 px-1.5 py-0.5 rounded-full">5 UC</span>
          </div>
        </div>
        <ThemeToggle />
      </div>
                  ...
                  <button key={s.id} onClick={() => setSelectedStation(s.id)} className={cn("flex items-center justify-between px-3 py-2.5 rounded-xl text-left transition-colors", selectedStation===s.id?"bg-primary/10 border border-primary/30":"bg-muted/40 hover:bg-muted")}>
                    ...
                    {isOp && crowd && selectedStation === s.id && (
                      <span className={cn("text-xs px-2 py-0.5 rounded-full",
                        ...
                        crowd.level==="moderate"&&"bg-blue-100 dark:bg-blue-900/30 text-blue-700",
                    ...
                    <div key={i} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                      <span className="text-muted-foreground">
                        {t.schedule.line === "blue"
                          ? (t.schedule.direction === "forward" ? "→ Thaltej Gam" : "← Vastral Gam")
                          : (t.schedule.direction === "forward" ? "→ Motera" : "← APMC")}
                      </span>
