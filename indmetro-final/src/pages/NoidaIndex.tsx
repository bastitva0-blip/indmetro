...
import { Train, ChevronRight, ArrowRight, AlertCircle } from "lucide-react";
import { useNavigate } from "react-router-dom";
import JourneyMode from "@/components/JourneyMode";
import { useJourneyTracker } from "@/hooks/use-journey-tracker-noida";
import { stations, getStationOptions, LINE_STATIONS, LINE_COLORS, OPERATIONAL_STATIONS } from "@/cities/noida/metroData";
import { planRoute, PlannedRoute } from "@/cities/noida/routePlanner";
import { getCrowdEstimate } from "@/cities/noida/crowdSimulation";
import { getNextTrainsAtStation } from "@/cities/noida/timetable";
import { useGoSmartCard } from "@/contexts/GoSmartCardContext";
import ThemeToggle from "@/components/ThemeToggle";
import OfflineIndicator from "@/components/OfflineIndicator";
import { cn } from "@/lib/utils";

function NoidaMap({ highlightIds, onStationClick }: { highlightIds: string[]; onStationClick: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((mod) => {
      const L = mod.default;
      const map = L.map(containerRef.current!, { center: [28.490, 77.430], zoom: 11, zoomControl: false });
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>', maxZoom: 19,
      }).addTo(map);
      L.control.zoom({ position: "bottomright" }).addTo(map);
      L.polyline(LINE_STATIONS.aqua.map(id => stations[id].coordinates as [number,number]), {
        color: LINE_COLORS.aqua, weight: 5, opacity: 0.9,
      }).addTo(map);
      Object.values(stations).forEach(s => {
        const marker = L.circleMarker(s.coordinates as [number,number], {
          radius: s.isInterchange ? 9 : 7, color: "#fff",
          weight: s.isInterchange ? 3 : 2, fillColor: LINE_COLORS.aqua, fillOpacity: 0.95,
        }).addTo(map);
  ...
  useEffect(() => {
    import("leaflet").then(() => {
      Object.entries(markersRef.current).forEach(([id, marker]) => {
        ...
        else marker.setStyle({ fillColor: LINE_COLORS.aqua, radius: s.isInterchange ? 9 : 7, weight: s.isInterchange ? 3 : 2 });
      });
    });
  }, [highlightIds]);
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <span className="text-xs bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 px-2 py-0.5 rounded-full font-medium">Aqua Line</span>
      </div>
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <select value={value} onChange={e => onChange(e.target.value)} className="bg-muted border border-border rounded-xl px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        {getStationOptions().map(s => <option key={s.id} value={s.id}>{s.name}{s.isInterchange ? " ↔" : ""}</option>)}
      </select>
    </div>
  );
}
...
export default function NoidaIndex() {
  ...
  const handleStart = useCallback(async () => {
    ...
    await startJourney(route.origin.id, route.destination.id, "aqua");
  ...
  return (
    <div className="h-[100dvh] w-full relative overflow-hidden bg-background">
      ...
      <OfflineIndicator />
      <NoidaMap highlightIds={highlightIds} onStationClick={handleMapClick} />
      ...
      <div className="absolute top-0 left-0 right-0 z-[1200] px-3 pt-3 flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-xl px-3 h-9 shadow">
          ...
          <span className="text-sm font-semibold">Noida Metro</span>
          <span className="ml-auto text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded-full">21 live</span>
        </div>
        <ThemeToggle />
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-[1100] bg-card border-t border-border rounded-t-2xl shadow-2xl">
        ...
        <div className="overflow-y-auto max-h-[55vh] px-4 pt-4 pb-8 space-y-4">
          {activePanel === "route" && (
            <>
              <div className="flex items-start gap-2 text-xs bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-2.5">
                <AlertCircle className="w-3.5 h-3.5 text-amber-600 mt-0.5 shrink-0" />
                <span className="text-amber-700 dark:text-amber-300">NMRC uses separate GoSmart CITY1 card — Delhi Metro cards not valid here.</span>
              </div>
              ...
              {getStationOptions().map(s => (
                <button key={s.id} onClick={() => setSelectedStation(s.id)} className={cn("flex items-center justify-between px-3 py-2.5 rounded-xl text-left transition-colors", selectedStation===s.id?"bg-primary/10 border border-primary/30":"bg-muted/40 hover:bg-muted")}>
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">🔵 Aqua Line{s.isInterchange?" · ↔ DMRC Blue (skywalk)":""}</p>
                  </div>
                ...
                <div className="mt-2 bg-muted/40 rounded-xl p-3">
                  ...
                  {nextFwd.map((t,i) => (
                    <div key={`f${i}`} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                      <span className="text-muted-foreground">→ Depot Station</span>
                  ...
                  {nextBwd.map((t,i) => (
                    <div key={`b${i}`} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                      <span className="text-muted-foreground">← Noida Sector 51</span>
                      <span className="font-medium tabular-nums">{t.minutesAway<1?"Now":`${Math.round(t.minutesAway)} min`}</span>
                    </div>
                  ))}
                </div>
